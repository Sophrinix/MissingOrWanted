...
@implementation MissingOrWantedObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_missingorwanted);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any MissingOrWanted patches in JS before a context is loaded... nice for 
...
{
	// create MissingOrWanted global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_missingorwanted = [[MissingOrWantedObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_missingorwanted];
	
	NSString *_missingorwantedNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _missingorwantedNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_missingorwanted valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.missingorwanted.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
