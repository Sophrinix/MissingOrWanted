...
// in simulator we redefine to format for MissingOrWanted Developer console
...
{\
	NSString * exceptionName = [@"org.missingorwanted." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.missingorwanted";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UIMissingOrWantedNativeItemNone = -1, 
	UIMissingOrWantedNativeItemSpinner = -2,
	UIMissingOrWantedNativeItemProgressBar = -3,
	
	UIMissingOrWantedNativeItemSlider = -4,
	UIMissingOrWantedNativeItemSwitch = -5,
	UIMissingOrWantedNativeItemMultiButton = -6,
	UIMissingOrWantedNativeItemSegmented = -7,
	
	UIMissingOrWantedNativeItemTextView = -8,
	UIMissingOrWantedNativeItemTextField = -9,
	UIMissingOrWantedNativeItemSearchBar = -10,
	
	UIMissingOrWantedNativeItemPicker = -11,
	UIMissingOrWantedNativeItemDatePicker = -12,
	
	UIMissingOrWantedNativeItemInfoLight = -13,
	UIMissingOrWantedNativeItemInfoDark = -14,
	
	UIMissingOrWantedNativeItemDisclosure = -15,
	
	UIMissingOrWantedNativeItemContactAdd = -16
};
